...
function productosDetallados(event) {
  ...
  $.ajax({
    url: 'http://localhost:45/Productos/'+ hijo.id,
    contentType: "application/json; charset=utf-8",
    async: true,
    type: 'GET',
    dataType: 'json',
    success: function (json) {
      $("#contenedor-mid-interior").html("");
        ...
        $("<div class='row panel-central'>" +
          ...
          "<img id='foto' class='img-fluid imagen-detallada' src=" + json.imagen + " alt=''>" +
          "</div>" +
          ...
          "<h2 class='nombre'>" + json.nombre + "</h2>" +
          "</div>" +
...
function guardarProductoDeseado() {
  ...
  miObjeto.idUsuario = String(id);
  miObjeto.idProducto = idProducto;
  var myString = JSON.stringify(miObjeto);
    

  if ($("#" + idProducto).hasClass("estrella-footer")) {
    $.ajax({
      url: "http://localhost:45/Deseado",
      contentType: "application/json; charset=utf-8",
      async: true,
      data: 
        myString,
...
async function productosDeseados() {
  ...
  return new Promise(function (resolve, reject) {   
      $.ajax({
        url: "http://localhost:45/Deseado/"+id,
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        async: true,
        dataType: 'json',
      ...
      success: function (json) {
        ...
        json.forEach(function callback(value){;
          var texto =
            ...
            '<td data-th="Product">' +
            '<div class="row">' +
            '<div class="col-sm-2 hidden-xs"><img src="' + value[3] + '" alt="..." class="img-responsive img-carro div-volver"/></div>' +
            '<div class="col-sm-8 td-texto--central">' +
            '<h4 class="nomargin">' + value[1] + '</h4>' +
            '<p>' + value[2] + '</p>' +
            '</div>' +
            '</div>' +
            '</td>' +
            ...
            '</td>' +
            '<td><big id=' + value[0] + ' class="boton-visible"><i class="fa fa-eye" aria-hidden="true"></i></big></td>' +
            '</tr>';
        ...
        //funcion que se ejecuta para cargar los iconos en rojo 
        comprobarMisproductos();
        
        //Se crean eventos onclick para los botones de eliminar y ocultar producto
...
function cambiarVisible() {
  ...
  if ($("big#" + idProducto + ".boton-visible").hasClass("boton-color")) {

    $("big#" + idProducto + ".boton-visible").removeClass("boton-color");
    visible= "0";
    
    $.ajax({
      url: "http://localhost:45/Miproducto/"+idProducto+"/"+visible,
      contentType: "application/json",
      async: true,
      type: "PUT",
      error: function (jqXHR, status, error) {
    ...
    $("<div class='col-lg-4 col-md-6 mb-4'>" +
      ...
      "</div>" +
      "<div class='card-footer card-footer-modificado'>" +
      "<big id =" + idProducto + "><i class='fa fa-heart' aria-hidden='true'></i></big>" +
      "</div>" +
      "</div>" +
      "</div>").appendTo("#contenedor-mid-interior");
    ...
    $("big#" + idProducto + ".boton-visible").addClass("boton-color");
    $("big#" + idProducto + ".corazon").parent().parent().parent().remove();
    visible= "1";
...
function comprobarMisproductos() {
  $.ajax({
    url: "http://localhost:45/ProductoVisible/"+ id,
    type: 'GET',
    dataType: "json",
    success: function (json) {
        json.forEach(n => {
...
function eliminarMiProducto() {
  var idProducto = this.parentNode.parentNode.children[2].children[0].id;
  var borrarProducto = this.parentNode.parentNode;
  $(borrarProducto).remove();
  $.ajax({
    url: "http://localhost:45/Miproducto/"+idProducto+"/"+id,
    type: "DELETE",
    dataType: 'json',
    contentType: "application/json",
    error: function (jqXHR, status, error) {
...
function rellenarSubcategorias() {
  ...
  $.ajax({
    url: "http://localhost:45/Subcategorias/"+valor,
    type: "GET",
    dataType: 'json',
    contentType: "application/json",
    async: true,
    success: function (json) {
      ...
      for (let i = 0; i < json.length; i++) {
        ...
        $('#subcategoria').append('<option value="' + json[i][0] + '">' + json[i][1] + '</option>');
      }
    }
  });
